...
<?php page_header(); ?>
<?php admin_menu(ADMIN_PAGE_INBOX); ?>
...
//  $.get("services/mother_info.php", function(data) {
//    $('#readMail').html(data.email);
//  }, 'json');
  // get the user's email.
  var getMail = function(type, reload) {
...
function getMothers() {
  $.get('services/mother_info.php', function(data) {
    var html = ''
    for (i in data) {
      html += '<option value="'+ data[i].mid +'">' + data[i].email + '</option>';
    }
    $('select[name="mailto"]').html(html)
  }, 'json');
}
...
function mailReply(element) {
  $('#composeTab').click(); 
  var mailDiv = $(element).parent('.mail');
  var from = mailDiv.find('span.mailFrom').html();
  var toSelect = $('select[name="mailto"]')[0] 
  for (i in toSelect.options) {
    if (toSelect.options[i].innerHTML == from) {
