...
$(function() {
  app = new MobileApp("Lactor", { 'scale':320 });
  base_url = 'https://lactor.org/';
...
//    },
  ...
  app.loadResource('js/legacy.js', 'js', function() {
  // enable legacy mode for android 2.x app.
    if (app.browser.android && app.browser.android.webview && 
        app.browser.android.version.major <= 2) {
      app.legacy();
    }
  });
  app.initViews({
    login: {
      ...
      init: function() {
        ...
        if (app.settings.get('username')) {
          $('#login_user').val(app.settings.get('username'));
        }
        $('button').on('click', function() {
          app.settings.set('username', $('#login_user').val());
    ...
    add_entry: {
      ...
      init: function() {
       ...
       $('#addentry_cancelButton').on(app.options.clickEvent, function() {
         app.loadView('add_entry');
       });
            ...
            (function(item) {
              if (item.status == 1) { //unread
                view.createItem({ 
                  ...
                  content: "<div class='notification-link'>" + 
                      lactor.notificationText[item.type-1].title + 
                      "<span class='notificationDate'>" +
                      new Date(item.issued * 1000).smartFormat() +
                      "</span></div><div style='clear:both;'></div>",
                  detail: function(callback) {
                    var text = lactor.notificationText[item.type - 1];
                    var html = '<span class="ndetail-problem">' + 'Problem: ' + 
                      text.title + "</span><br>" + '<span class="ndetail-date">' + 
                      new Date(item.issued * 1000).smartFormat() +  '<br>' +
                        '</span>';
